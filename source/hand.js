  ...
  tick: ({ context, hand, world, queue }) => {
    ...
    if (Mouse.Left) {
      ...
      if (pick.address && pick.part.type === PART_TYPE.EDGE) {
        ...
        hand.startDrawnParent = getDrawnScreenFromRoute(
          pick.route,
          pick.route.length - 2
        );
      ...
      if (pick.address && pick.part.type === PART_TYPE.CORNER) {
        ...
        hand.startCorners = getDrawnScreenFromRoute(pick.route).corners;
        hand.startPosition = getCornersPosition(
          hand.startCorners,
          hand.pick.part.number
        );
        hand.startDrawnParent = getDrawnScreenFromRoute(
          pick.route,
          pick.route.length - 2
...
HAND_STATE.MOVING = {
  ...
  tick: ({ context, hand, world, queue, colours }) => {
    ...
    if (!hand.hasChangedParent && newPick.isWithinParent) {
      const newDrawnParent = getDrawnScreenFromRoute(
        hand.pick.route,
        hand.pick.route.length - 2
      );
      const parentDifferences = getSubtractedCorners(
        oldDrawnParent.corners,
        newDrawnParent.corners
      );
...
HAND_STATE.ROTATING = {
  ...
  tick: ({ context, hand, world, queue, colours }) => {
    ...
    // Work out screen movement
    const movedPosition = addVector(
      hand.startPositions[pick.part.number],
      movement
    );
    const movedCorners = getRotatedToPositionCorners(
      hand.startCorners,
      pick.part.number,
      movedPosition
    );
