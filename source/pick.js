...
import {
  getAddressFromScreen,
  getScreenFromAddress,
  makeAddress,
} from "./address.js";
...
export const pickInScreen = (screen, position, options = {}) => {
  ...
  if (!snapped && depth < maxDepth) {
    ...
    for (const child of screen.colour.screens) {
      ...
      if (
        child === ignore &&
        (ignoreDepth === undefined || ignoreDepth === depth + 1)
      ) {
        continue;
      }

      const scaledPity = getScaledPosition(pity, child.corners).map((axis) =>
        Math.abs(axis)
      );
      ...
      if (childPart.type === PART_TYPE.OUTSIDE) {
        ...
        maxBruteForce--;
        const relativeCorners = getRelativePositions(
          child.corners,
          screen.corners
        );
        ...
        const addressedScreen =
          address === undefined ? screen : getScreenFromAddress(address);
  ...
  let pickedScreen =
    parent === undefined ? screen : parent.colour.screens[number];
...
export const placeScreen = (screen, target, options = {}) => {
  const picks = screen.corners.map((corner) =>
    pickInScreen(target, corner, { ...options, bruteForceDepth: 0 })
  );
  const [head] = picks;
...
export const replaceAddress = ({
  ...
  let picks = targetedScreen.corners.map((corner) =>
    pickInScreen(target, corner, pickOptions)
  );
  let isStillWithParent = picks.some((pick) => pick.screen === parent);

  if (!isStillWithParent) {
    const snapPicks = targetedScreen.corners.map((corner) =>
      pickInScreen(target, corner, { ...pickOptions, snap: parent })
    );
    const snapIsStillWithParent = snapPicks.some(
      (pick) => pick.screen === parent
    );
  ...
  let resultParent =
    pickLeader.address === undefined
      ? pickLeader.screen
      : getScreenFromAddress(pickLeader.address);
...
export const tryToSurroundScreens = (address) => {
  ...
  for (let i = 0; i < length; i++) {
    ...
    const mappedChildCorners = getMappedPositions(
      child.corners,
      screen.corners
    );
    const insideScreen = mappedChildCorners.every((corner) =>
      isMappedPositionInCorners(corner)
    );
